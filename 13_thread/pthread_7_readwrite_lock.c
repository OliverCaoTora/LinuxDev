...
/*
为什么读锁可以多个线程同时拿？因为大家只是读，不会改数据，所以互相不影响。

但写锁必须独占。因为如果一个线程正在修改数据，其他线程同时读或写，都可能看到不一致的数据。

和 mutex 对比：

锁类型	特点
pthread_mutex_t	同一时刻只允许一个线程进入，不区分读写
pthread_spinlock_t	同一时刻只允许一个线程进入，等待时自旋
pthread_rwlock_t	多个读者可同时进入，但写者独占
*/
